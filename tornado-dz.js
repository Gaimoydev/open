...
function customStringify(e) {
    ...
    if (Array.isArray(e)) {
        return `[${e.map(item => (undefined === item ? "null" : customStringify(item))).join(",")}]`;
    }
    return `{${Object.keys(e).filter(t => e[t] !== undefined && "function" != typeof e[t] && "symbol" != typeof e[t]).sort().map(t => `${JSON.stringify(t)}:${customStringify(e[t])}`).join(",")}}`;
}

function getDynamicHeaders(method, path, body = null) {
    const methodUpper = method.toUpperCase();
    const bodyContent = customStringify(body);
    const bodyHash = sha256(bodyContent);
    const signBase = `${methodUpper}\n${path}\n${bodyHash}`;
    const ts = Math.floor(Date.now() / 1000).toString();
    const encodedParams = base64UrlEncode(signBase);
    const sign = sha256(`${signBase}${ts}`);

    return {
        "x-api-ts": ts,
        "x-api-userid": "0",
        "x-api-params": encodedParams,
        "x-api-sign": sign
    };
}
// ==========================================
...
function buildRequest(_0x2a7d, _0x55819b) {
  ...
  // ==========================================
  // === 新增：为 HTTP/1.1 添加动态签名头 ===
  // ==========================================
  const dynamicHeaders = getDynamicHeaders(reqmethod, url.pathname, null);
  _0xb9eacc += `x-api-ts: ${dynamicHeaders["x-api-ts"]}\r\n`;
  _0xb9eacc += `x-api-userid: ${dynamicHeaders["x-api-userid"]}\r\n`;
  _0xb9eacc += `x-api-params: ${dynamicHeaders["x-api-params"]}\r\n`;
  _0xb9eacc += `x-api-sign: ${dynamicHeaders["x-api-sign"]}\r\n`;
  // ==========================================
      ...
      _0x2f_0x913 = tls.connect({
        ...
        function _0x59adeb() {
          ...
          for (let i = 110581 ^ 110581; i < _0x4ca; i++) {
            ...
            // === 新增：为 HTTP/2 添加动态签名头 ===
            // ==========================================
            const actualPath = query ? _0xcf83bc(query) : url.pathname + (postdata ? `?${postdata}` : "");
            const dynamicHeaders = getDynamicHeaders(reqmethod, actualPath, null);
            ...
            const _0xcaf21b = Object.entries({
              ...
              // 植入我们生成的请求头
              "x-api-ts": dynamicHeaders["x-api-ts"],
              "x-api-userid": dynamicHeaders["x-api-userid"],
              "x-api-params": dynamicHeaders["x-api-params"],
              "x-api-sign": dynamicHeaders["x-api-sign"],
              // ==========================================
